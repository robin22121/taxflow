...
## 6. 급여 공제 4대보험 입력 방식 — 고정액 + 연간 정산 vs 실지급액 계산 (2026-09-25 분석, 미착수)

### 6-1. 실무

| 보험 | 월 공제 기준 | 정산 |
|------|------------|------|
| 국민연금 | 사원 **기준소득월액**(취득·변경 신고한 값) × 요율 — 매월 같은 금액 | 없음 (기준소득월액이 바뀔 때만 변경 신고) |
| 건강보험·장기요양 | 업체마다 다름 (아래 A·B) | 연 1회 보수총액으로 정산 (정산보험료) |
| 고용보험 | 업체마다 다름 (아래 A·B) | 연 1회 보수총액 신고로 정산 |

- **A. 고정액 + 연간 정산**: 국민연금처럼 신고한 보수월액 기준으로 매월 같은 금액(고지액과 같게)을 공제하고, 다음 해 정산보험료로 차액을 맞춘다
- **B. 실지급액 계산**: 정산을 피하려고 매월 실제 급여 × 요율로 계산해 공제한다

### 6-2. 이지원천 현재 (코드 실측)

- 4대보험 모두 **당월 과세급여 × 요율**로 계산 (`services/tax_calc.py` `calculate_social_insurance_with_overrides`, `api/collect.py:787-798`). 급여대장에 값이 있으면 그 값 우선
- 즉 건강·고용은 사실상 **B만** 지원하고, 국민연금도 기준소득월액이 아니라 당월 급여로 계산한다 → 급여가 달마다 바뀌는 사원은 국민연금이 실무(고정액)와 달라진다
- 사원별 **기준소득월액·보수월액 칸이 없다** (`Employee`, `PayrollEntry` 모두)
- 거래처 설정(`ClientPayrollDefault`)은 보험별 적용 여부·요율 예외만 있고 **A/B를 고르는 설정이 없다**
- 정산보험료(`PayrollEntry.settlement_insurance`, 엑셀 S열)는 급여대장 값을 받는 칸일 뿐 계산하지 않는다
- ⚠️ **국민연금 상·하한 불일치**: `tax_calc.py`는 370,000~5,900,000, `insurance_excel.py:40-41`·이 문서는 400,000~6,370,000 (2025.7~2026.6). 즉시 수정 후보

### 6-3. 필요한 작업

1. **사원별 기준 금액 칸** — `Employee`에 국민연금 기준소득월액, 건강보험 보수월액, 고용보험 월평균보수(선택). 위하고 사원등록에서 가져오기(`16-wehago-rpa.md` §12) 또는 4대보험 취득·변경 신고 값으로 채운다
2. **거래처 설정: 건강·고용 공제 방식** — `ClientPayrollDefault`에 A(고정액)/B(실지급액) 선택. 기본값은 A? B? — 사무소 관행 조사 후 결정
3. **계산 분기**
   - 국민연금: 항상 기준소득월액 × 요율 (값이 없을 때만 당월 과세급여로 대체하고 “기준소득월액 미입력” 경고)
   - 건강·장기요양·고용: A면 보수월액 × 요율, B면 당월 과세급여 × 요율 (지금 방식)
4. **정산 월 처리** (A 거래처) — 건강보험 정산보험료(보통 4월 고지)·고용보험 정산분을 고지서 값으로 입력받아 정산보험료 열에 넣는다. EDI 고지 연동은 이 문서 §2 운영 모델과 함께 검토
5. **검증** — A 거래처는 매월 공제액이 4대보험 고지액과 같아야 한다. 고지서(또는 EDI 고지 내역)와 급여 공제액을 대조해 다르면 알린다. 위하고 쪽은 사원등록 보수월액 × 요율과 엑셀 값을 비교할 수 있다 (`16-wehago-rpa.md` §9-4 (c)와 같은 방식)
6. 국민연금 상·하한 상수 한 곳으로 통일
