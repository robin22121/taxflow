...
| 2026-09-25 | **운영 경로 = 이지원 홈택스 자동 로그인 확정** (§3-9-7-4) — 이지원(사무소 자동화 노트북)에 세무법인 공동인증서와 세무사 아이디·비밀번호·인증서 비밀번호·세무대리 관리번호를 저장해 두고, 에이전트가 **Playwright 로 로그인부터 발급까지** 사람 없이 진행한다. 사람이 로그인해 둔 Chrome 에 부착하는 방식(§3-9-7-3)은 **개발 전용**으로 한정 | 사용자가 이지원에서 인증서 선택·비밀번호 입력을 포함한 자동 로그인이 Playwright 로 되는 것을 실제 확인. 수동 로그인은 매일 아침 로그인·무활동 로그아웃 후 재로그인이 필요해 직원 발급 요청의 가장 큰 병목이 된다 |
...
> **개발 전용.** 운영(이지원)은 자동 로그인이다 (§3-9-7-4, 2026-09-25).
...
- **최종 구현**: 발급 구간이 굳으면 로그아웃 상태에서 자동 로그인(§3-9-7-4) → 발급까지 처음부터 끝까지 재검증한다.

### 3-9-7-4. 운영 경로 — 이지원 홈택스 자동 로그인 (2026-09-25 확정)

최종 목표는 **사람이 로그인하지 않는 것**이다. 이지원에 인증서와 로그인 정보를 저장해 두고 에이전트가 Playwright 로 로그인한다 (사용자 실측 확인).

**이지원에 두는 것**
- 세무법인 **공동인증서** — 이지원 하드디스크에 복사 (홈택스 인증서 선택창에 나타나는 위치)
- Windows 자격 증명 관리자(DPAPI, `setup`): 홈택스 아이디·비밀번호 · **공동인증서 비밀번호** · 세무대리 관리번호·비밀번호 (`plan/16-wehago-rpa.md` §8-2 저장 대상과 동일, 서버 저장 금지)

**로그인 순서** (`plan/16-wehago-rpa.md` §9-2 실측 경로)
아이디 로그인 → 공동인증서 선택 + 인증서 비밀번호 → "세무대리 관리번호로 로그인" → 관리번호·비밀번호

**에이전트 동작**
1. 증명원 작업을 claim 하면 먼저 홈택스 탭의 로그인 상태를 확인한다 ('로그아웃' 버튼 유무).
2. 풀려 있으면 저장된 정보로 **자동 로그인** 후 발급 구간(`issue_certificate`)으로 넘어간다. 아침 첫 작업·무활동 로그아웃 뒤에도 사람 개입이 없다.
3. 로그인 실패는 **1회에 즉시 에이전트 정지** + 작업 실패 회신 (비밀번호·인증서 비밀번호 반복 오류로 계정·인증서가 잠기는 것 방지 — 위하고 에이전트와 같은 정책). 사용자는 `setup` 으로 정보를 고친 뒤 재시작한다.
4. 브라우저는 **홈택스 전용 Chrome**(전용 프로필·CDP 9223)으로 띄워 위하고·위택스와 탭을 공유하지 않는다 (위하고 에이전트는 첫 번째 탭을 가져다 쓰므로 같은 Chrome 이면 홈택스 탭을 덮어쓸 수 있다). 원천세 홈택스 신고도 같은 홈택스 줄에 서서 증명발급과 순서대로 처리한다 — 로그인 유지·연장·로그아웃 원칙은 `plan/16-wehago-rpa.md` §8-3.

**남은 구현**
- `certificate_agent easyone` 경로에 자동 로그인 단계 연결 (지금은 CDP 부착만 — 로그인이 풀리면 "홈택스 로그인이 풀렸는지 확인하세요"로 실패)
- `easyone` 경로의 자격증명을 keyring(`setup`)으로 받기 — 지금은 토큰만 옵션·환경변수
- 위하고 에이전트 `rpa-agent/easyone_agent/hometax.py` 의 "인증서 팝업은 Playwright 로 처리 불가" 전제(모듈 docstring·`HometaxCertRequired`)를 자동 로그인으로 교체 — 홈택스 로그인 코드는 두 에이전트가 하나를 공유
...
- 화면 하단 고정 바. **칩 = 대기·진행 중 + 오늘(한국 시간) 끝난 작업**(오늘 끝난 작업은 [확인] 누르면 내려감).
  **맨 오른쪽 [전체작업↔내작업] 토글 버튼 하나** — 누를 때마다 전체작업 → 내작업 → 전체작업 순으로 바뀌고 칩 범위가 따라 바뀐다(팝업 없음).
  그 옆 **[내역보기]** → 작업 내역 팝업: 토글에서 고른 범위, **기본 최근 3일**, [1주일]·[한 달]로 기간 변경 (`days=3|7|30`, 최대 31일).
  (2026-09-25 변경 이력: 맨 왼쪽 [전체 작업 내역] → 오른쪽 [전체작업 | 내작업] 두 버튼 → 토글 1개 + [내역보기])
  예) `상도기업 원천세 신고서 입력중` → `원천세 신고완료` → `납부서 전송완료`,
  `엄순덕님 증명서 발급 진행중` → `발급 완료` → `고객발송 완료`.
- **전체작업** = 사무소 직원 전체의 작업 내역·대기 내역, **내작업** = 내가 요청한 작업만.
  다른 직원 작업은 **직원 이름 + 업무명**만 보인다 (예: `김동료 증명서 발급 대기`). 거래처명·사업자번호·귀속월·결과 메시지·단계 상세는
  서버(`GET /api/v1/rpa/jobs/activity?scope=all|mine`)가 비워서 보낸다 — 화면에서만 가리지 않는다.
- 칩 클릭 → 상세 팝업, [확인] 누르면 바에서 사라짐 (`rpa_jobs.acknowledged_at`, 사무소 단위). 작업 내역에는 남는다. [확인]·[진행 보기]는 **내 작업에만** 보인다.
...
- 에이전트 claim 분리: 위하고 claim(`/rpa/agent/claim`)은 증명원 작업을 가져가지 않고, 증명원은 `/certificates/agent/claim` 으로만 나간다.
  동시 실행 제한도 종류별(위하고 1건 · 증명원 1건)이다 — §4-4 의 "원천세·증명발급 상호 mutex" 는 2026-09-25 **사이트 세션별 제한**으로 구체화 — 홈택스 신고·증명발급은 한 줄 (`plan/16-wehago-rpa.md` §8-3, 서버 claim 미반영).
...
- 에이전트 `certificate_agent easyone` (README) — 개발 경로(CDP 부착)만. 운영 경로(이지원 자동 로그인, §3-9-7-4)는 미구현.
