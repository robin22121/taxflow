...
- 코드 구조: `issue_flow.issue_business_registration(context, page, job)` 가 로그인 이후 공통 구간.
  최종 경로 `execute_phase1` = 브라우저 기동 + `login()` + 공통 구간,
  개발 경로 `execute_attached` = CDP 부착 + 공통 구간. **새 증명원 종류도 공통 구간 함수로 추가**하면
  최종 경로에 그대로 들어간다.
...
- 맥북(macOS 13 arm64)에서도 동작 확인 — Playwright 번들 chromium 이 아니라 시스템 Chrome 에 붙기 때문.
  CDP 연결 → 홈택스 탭 탐지 → 비로그인 경고 → 연결 해제 후 탭 유지까지 확인 (2026-09-22, 로그인 전 상태).
