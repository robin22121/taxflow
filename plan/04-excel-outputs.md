...
> plan.md §3.3, §3.5 분리본. 급여대장 양식 엑셀 생성 + 간이지급명세서·급여명세서·4대보험 신고서 등 출력 양식 전반.

> ⚠️ **문서-코드 드리프트**: 아래 §1은 SmartA 24컬럼 기준으로 쓰여 있으나, 현재 `payroll_excel.py`는
> **위하고T 22컬럼**(수당 5 + 지급액계 + 공제 9 + 공제액계 + 차인지급액, 인적사항 5)으로 구현돼 있다.
> 기준 실측 파일은 `(임시)주식회사 동문-202511.xlsx`이며 `tests/test_payroll_excel.py`가 이를 고정한다.
> §1 의사코드 갱신은 별도 과제.
...
**4-3. 사업소득 SmartA 일괄등록 양식** (구현 완료 — `smarta_business_xls.py`)
- 더존 SmartA 인사급여 > 사업소득자료입력 메뉴가 읽는 변환 서식(`saup_excel.xls`) 재현
- A열 일련번호 + 14항목: 귀속년월 / 지급년월일 / 소득자명 / 주민등록번호 / 기본주소 / 상세주소 /
  소득구분 / 영수일자 / 지급총액 / 세율(%) / 소득세 / 지방소득세 / 내.외국인구분 / 연말정산
- **소득구분은 업종코드가 아니라 한글 라벨 40종** (병의원·저술가·학원강사 …). IV열(255)에 목록 동봉
- 업종코드 40종 ↔ SmartA 라벨 1:1 매핑, 세율은 `BUSINESS_TYPE_CODES` 재사용(봉사료수취자만 5%)
- BIFF(.xls) 포맷이라 openpyxl 불가 → **`xlwt` 의존성 추가**. ⚠️ 배포 시 vm-node에도 설치 필요
- 기본주소·상세주소는 `Employee`에 필드가 없어 빈칸(서식상 필수 아님).
  내외국인구분·연말정산도 빈칸이면 각각 내국인·부로 처리됨
- 위 2·4번 간이지급명세서(홈택스 제출용)와는 **별개 산출물** — 이쪽은 SmartA 업로드용

### 통합 다운로드 (구현 완료)

`GET /filings/{id}/unified-download` — 단일 **ZIP**. 프론트는 통합 다운로드 클릭 시
**거래처 선택 모달**을 띄우고, 선택된 `client_ids`(쉼표 구분)를 넘긴다.

```
통합신고자료_2025-11.zip
├─ 하늘식품/
│   ├─ 급여대장_2025-11.xlsx
│   ├─ 4대보험_통합_2025-11.xlsx
│   └─ 사업소득_지급명세서_2025-11.xls   ← 사업소득 항목이 있을 때만
└─ 동문건설/
    └─ ...
```

- **거래처별 폴더 분리가 핵심** — 한 파일에 여러 거래처를 섞으면 SmartA·위하고T 업로드 시
  다른 회사 직원이 함께 등록된다. 급여대장 22컬럼에 거래처 구분 컬럼이 없어 사후 분리도 불가
- 폴더명: 상호에서 경로 구분자·제어문자 제거, 중복은 `_2` 접미사, 빈 상호는 `거래처`, 60자 컷
- 타 세무사무소 거래처 id가 넘어오면 `tax_office_id` 검증으로 건너뜀
- 파일을 개별로 내려받으면 **브라우저의 다중 자동 다운로드 차단에 걸려 조용히 유실**되므로 ZIP 필수
- 미수신·확인필요 거래처는 모달에 배지와 인라인 경고로 표시(기존 `window.confirm` 대체)
