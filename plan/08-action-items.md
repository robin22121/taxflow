...
### [ ] 발송 확장 — 신고안내·납부서·고지서 (2026-09-22) — ⚠️ **미구현**

설계: `plan/13-messaging-activation.md` §5. 세목 4종(부가세·종합소득세·법인세·원천세).

- [ ] **P0** 사이드바 "문자발송" 메뉴 — 클릭 시 문자/카톡(알림톡) 선택, 이력·템플릿·채널 설정 하위
- [ ] **P0** 세목 선택형 신고안내 (원천세 신고안내 포함) + 예약 발송 스케줄러 + 수집 세션과 분리된 발송 로그
- [ ] **P0** `Client` 세목 필드 — 부가세 과세유형·원천세 반기납부·법인 결산월·성실신고 대상 (`plan/02-data-model.md` 반영)
- [ ] **P1** 범용 알림톡 템플릿 T4(신고안내)·T5(납부안내) 심사 — 착수 전 Aligo 발신프로필 그룹 템플릿 공유 가능 여부 확인
- [ ] **P2** 납부서 발송 — 원천세는 `ClientFilingResult`+포털 PIN 링크 연결, 타 세목은 PDF 일괄 업로드 → 사업자번호 매칭
- [ ] **P3** 홈택스 고지·미납 조회 → 발송 — 세무대리인 조회 범위 실무 검증 선행
- **전제**: 위 "개별 세무사사무소 발신번호로 SMS 발송" 항목 선행
