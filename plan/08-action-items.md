...
### [ ] 세무서 관할 담당자 디렉토리 — ⚠️ **미구현**
- **배경**: 세무서는 법정동 단위로 담당자를 배정(예: 강서구 등촌동=A, 목동=B). 세무사 사무실 직원은 문의할 때마다 대표번호 → 담당 확인 → 연결을 반복 → 시간 손실
- **목표**: 사업자 소재지(법정동) → 관할 세무서 + 담당자(이름·직위·직통번호) 자동 매칭. 거래처 상세/신고 화면에서 "관할 담당자에게 전화" 원클릭
- **수집 전략(하이브리드)**:
  1. **1차 — 공개 자료 크롤링**: 국세청/각 세무서 홈페이지 "담당구역 안내·조직도" 스크래핑 (공개된 곳부터 커버)
  2. **2차 — 정보공개청구 보완**: 미공개 세무서만 매년 1월 청구. 단 공무원 개인 연락처는 부분/비공개 위험 있어 "부서·직위·업무분장·대표 직통" 수준으로 요청 문안 조율
  3. **3차 — 사용자 crowdsource**: "이 담당자 정보 최신인가요?" 피드백 + 유저가 통화 후 담당자 갱신. 인사이동은 수시 발생하므로 연 1회 갱신만으로는 stale
- **데이터 모델(안)**: `TaxOfficer(id, tax_office_code, name, position, duty, phone, email, updated_at, source)`, `TaxOfficerJurisdiction(officer_id, sido, sigungu, dong_bjd_cd)`. Client 주소 → 법정동 코드 매칭
- **우선순위**: 3게이트 RPA(홈택스·위택스 인증서 발급/신고) 코어 라인 이후. Phase 3 이후 부가 기능으로 편입
- **법적 고려**: 정보공개법상 업무정보(부서·직위·업무·연락처)는 통상 공개, 개인정보(휴대폰 등)는 비공개 사유 가능. 대량 스크래핑·재판매 이미지 주의

### [ ] 사무소별 카카오 채널 대행 (§9-4 결정 파생, 2026-09-21) — ⚠️ **미구현**

`plan/18-kakao-inbound.md` §9 채택 · `plan/13-messaging-activation.md` §3.2 전환에서 파생된 4개 실행 항목. 유료화·신규 유치 개시 전 필수.

- [ ] **`TaxOfficeKakaoChannel.template_approval_status` JSON 스키마 확정**
  - 사무소별 템플릿 코드(T1·T2·T3) × 승인 상태(PENDING·APPROVED·REJECTED·REVOKED)·심사 신청일·반려 사유 저장 구조
  - 자동 폴링 없음 — 이지원천 온보딩 담당자가 카카오 심사 결과를 보고 수동 갱신
  - `plan/02-data-model.md` 반영 필요

- [ ] **신규 유치 온보딩 SOP 문서 작성**
  - 이지원천 담당자가 세무사 대신 카카오 채널 개설·비즈 인증·senderKey 발급·템플릿 3종 심사 신청·반려 대응·관리자 권한 이양을 수동으로 진행하는 절차
  - 각 단계 소요·필요 서류·연락처(카카오·Aligo)·반려 사유별 문구 수정 가이드
  - 온보딩 완료 후 `TaxOfficeKakaoChannel` 레코드 생성·`template_approval_status` 초기값 세팅

- [ ] **사무소 배포용 매니저센터 응대 가이드 작성**
  - 당번제 운영 권장 (그날의 당번 직원 1명이 인바운드 문의 일괄 처리, 필요 시 오프라인 인계)
  - 사장님이 채팅에 개인정보(주민번호·계좌·인증서 PW)를 남긴 경우 대응 — 즉시 삭제 요청, 사업주 포털로 유도
  - 이지원천이 마스킹 도구·SLA 모니터링·라우팅 시스템을 제공하지 않음을 명시

- [ ] **이용계약서 카카오 채널 대행 위임 조항 초안 작성 + 법무 검토**
  - 위임 범위(채널 개설·비즈 인증·senderKey·템플릿 심사·관리자 권한 이양)
  - senderKey 폐기 절차 (사무소 이탈·유출 시)
  - 사무소 이탈 시 채널 소유권 명시 (채널은 사무소 자산으로 잔존)
  - 개인정보 처리 조항 — 이지원천이 senderKey만 취급, 인바운드 대화 이력은 저장하지 않음
  - `plan/10-privacy-security.md` 관점 신규 저장 대상(senderKey) 등록

### [ ] 식대 제출비과세(2023 귀속~) 반영 — ⚠️ **미구현**
- **배경**: 2023 과세기간부터 식대(월 20만원)가 지급명세서 **제출비과세**로 변경. 자가운전보조금은 미제출비과세 유지, 육아(보육)수당은 제출비과세
- **영향 없음**: 원천세·4대보험 계산(`taxable = 총지급액 − 비과세 합계`)은 제출/미제출 구분과 무관 — 수정 불필요
- **수정 필요**:
  1. 간이지급명세서(근로소득) `비과세소득` 칸에 자가운전보조금까지 합산 기재 (`simple_statement_excel.py:96`, `entry.non_taxable`) → **식대 + 육아수당(제출비과세)만** 기재
  2. `급여` 칸에 비과세 포함 총지급액이 들어가 비과세 이중 기재 (`simple_statement_excel.py:81`, `salary_amount = total_amount` — `collect.py:742`, `imports.py:393`) → 과세분만 기재
  3. 식대(및 자가운전·육아) 월 20만원 한도 미검증 — 원시자료/수동수정 값을 전액 비과세 처리 (`collect.py:700`, `filings.py:697`) → 초과분 과세 전환 여부 결정 후 구현
- **참고(관점 밖)**: 위하고T 일괄등록 엑셀은 비과세 합계 한 칸뿐(`wehago_excel.py:81`, 양식 미확정), 마스터 임포트는 비과세 합계만 받아 식대 분리 불가(`imports.py:386`)
- **전제 확인**: 간이지급명세서 비과세 칸 = 제출비과세만 기재(미제출비과세 미기재) 기준
