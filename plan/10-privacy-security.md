...
3. ~~SmartA RPA 에이전트 아키텍처 설계 (pywinauto)~~ → **위하고 T·홈택스·위택스 자동화 노트북 RPA로 변경** (2026-09-15, [`16-wehago-rpa.md`](16-wehago-rpa.md)) — 봇 감지·화면 실측 PoC 필수
...
- [ ] **G5. RPA 부산물 차단 가드** (Phase 2, [`16-wehago-rpa.md`](16-wehago-rpa.md) §8) — 🟡 설계 반영, 구현 일부
  - 자동화 노트북 에이전트: 급여대장 임시파일은 업로드 후 즉시 삭제(구현됨), 로그·실패 스크린샷에 비밀번호·주민번호 기록 금지(입력칸 가림), 클립보드 미사용
  - 위하고·홈택스·위택스 로그인 정보는 노트북 Windows 자격 증명 관리자에만 암호화 저장 (서버 저장 금지), 노트북 VeraCrypt 시스템 암호화(Windows Home 포함)·전용 Windows 계정·자동 로그온 끄기
  - 사원등록(주민번호 포함 인적사항)은 자동화 제외 — 직원이 위하고에 직접 입력
...
- **G5 (RPA 부산물)**: Phase 2 사안, 그대로 유효 (대상이 SmartA에서 자동화 노트북으로 바뀜 — `16-wehago-rpa.md` §8)
