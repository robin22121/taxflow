...
### 2.0 주민번호 사용 원칙 — 계층 분리 (2026-09-23 재정의)

세무사무소가 원천세·지급명세서(근로/일용/사업/기타)를 신고하려면 대상자 주민번호가 필요하다. 사업소득·기타소득 지급명세서는 **주민번호+지급액이 한 장에** 들어오는 게 표준이라 "탐지 시 삭제/차단"으로는 신고 자체가 성립하지 않는다. 반면 외부 LLM으로 평문 RRN이 나가는 것은 어떠한 경우에도 허용되지 않는다. 따라서 세 계층을 분리해 동시 성립시킨다:

1. **저장 계층 (서버 내부)** — 신고에 필요한 RRN은 파일·메시지 반입 지점에서 **결정론 파서로 추출** → 검증 대칭키 알고리즘으로 암호화 → `Employee.rrn_encrypted` 에 저장, 표시용 `rrn_last4` 파생. 원본 평문은 DB·로그·파일 시스템 어디에도 남기지 않는다. 반입 채널을 가리지 않음 — 마스터 업로드든, 급여파일 업로드든, 카톡 첨부든, 이메일 첨부든, RRN 이 파일에 있으면 서버가 그 자리에서 뽑아 저장한다.
2. **송신 계층 (외부 LLM)** — LLM 프롬프트로 나가는 모든 텍스트는 `services/pii.py::redact_pii` 로 마스킹 (`900101-1234567` → `******-*******`). LLM 은 원본 RRN 을 절대 볼 수 없다. AI 파서의 입력 스키마·출력 스키마 모두 RRN 필드를 두지 않는다 — 서버 사이드채널로 별도 병합한다.
3. **표시 계층 (UI·외부 노출)** — 화면·알림톡·이메일 본문에는 `rrn_last4` 기반 마지막 4자리만 노출 (`******-*4567`). 원본 RRN 복호화는 신고 산출물 생성 시점(간이지급명세서·EDI XML 등) 에 한해 메모리에서만 이뤄지고, 산출물 파일 저장 이후엔 즉시 폐기한다.

이 원칙 위에서 G3(송신 스크러빙)와 G4(결정론 추출·저장)는 서로 상충하지 않고 **같은 파일 안에서 원본은 서버로, 마스킹본은 LLM으로 분기**하는 형태로 동시 성립한다.
...
- [ ] **G4. 평문 수집 채널 RRN 결정론 추출·저장** — ⚠️ 미구현 (2026-09-23 재정의)
  - 보안 입력 URL 외 카톡/이메일/업로드(엑셀·CSV·PDF·이미지)로 들어온 평문 RRN은 반입 지점에서 **결정론 파서로 추출** → `Employee.rrn_encrypted` 에 암호화 저장, 표시용 `rrn_last4` 파생. 채널·소득구분(근로/일용/사업/기타) 구분 없이 동일 원칙 적용
  - LLM 프롬프트에는 여전히 `redact_pii` 로 마스킹된 텍스트만 전달 (G3). AI 파서 스키마엔 RRN 필드 없음 — 서버 사이드채널로 `{이름 → rrn_encrypted, rrn_last4}` 맵을 만들어 파싱 결과에 후병합
  - 메시지 저장소·로그·임시 파일에 원본 평문 잔존 금지. 파일 원본은 저장하되 접근 로그 남기고, 텍스트 덤프·로그 라인은 마스킹본만 남긴다
  - 배경: 사업소득/기타소득 지급명세서는 주민번호+지급액이 한 장으로 들어오는 게 표준. "탐지 시 삭제"로는 신고 자료가 성립하지 않음 (§2.0 원칙)
