...
## 8. 홈택스 공동인증서 준비 (자동 로그인, 첫 실행 전)

홈택스는 **에이전트가 자동 로그인**한다 (2026-09-25 이지원에서 Playwright 동작 확인, `plan/17-certificate-issuance.md` §3-9-7-4). 순서: 아이디 로그인 → 공동인증서 선택 + 인증서 비밀번호 → 세무대리 관리번호 로그인.

준비할 것:
1. 세무법인 **공동인증서를 이 노트북 하드디스크에 복사**한다 (홈택스 인증서 선택창에 보이는 위치).
2. 전용 크롬에서 홈택스에 **한 번 직접 로그인**해 보고, 보안 프로그램 설치 안내가 뜨면 설치한다 (설치 확인용 — 이후 로그인은 에이전트가 한다).
3. §9 `setup` 에서 홈택스 아이디·비밀번호, 세무대리 관리번호·비밀번호, **공동인증서 비밀번호**를 저장한다.

로그인에 1회 실패하면 에이전트는 스스로 멈춘다 (비밀번호 반복 오류로 계정·인증서가 잠기는 것 방지). `setup` 으로 정보를 고친 뒤 다시 실행한다.

> 구현 상태: 자동 로그인 코드는 아직 에이전트에 연결 전이다. 연결 전까지는 위 2번처럼 직접 로그인해 둔 세션을 에이전트가 이어 쓴다.
